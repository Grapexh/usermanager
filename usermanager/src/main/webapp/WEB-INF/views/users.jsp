...
<body>
    ...
    <table class="easyui-datagrid" id="userList" title="会员列表" 
	       data-options="singleSelect:false,collapsible:true,pagination:true,url:'/rest/user',method:'get',pageSize:5,toolbar:toolbar,pageList:[2,5,10]">
	    <thead>
	        <tr>
...
<div id="userAdd" class="easyui-window" title="新增会员" data-options="modal:true,closed:true,iconCls:'icon-save',href:'/user/page/user-add'" style="width:450px;height:350px;padding:10px;">
        The window content.
</div>
<div id="userEdit" class="easyui-window" title="修改会员" data-options="modal:true,closed:true,iconCls:'icon-save',href:'/user/page/user-edit'" style="width:450px;height:350px;padding:10px;">
        The window content.
</div>
    ...
    handler:function(){
    	$('#userEdit').window({    
    	    onLoad:function(){
    	    	var userList = $("#userList");
    	    	var sels = userList.datagrid("getSelections");
    	    	var birthday = new Date(sels[0].birthday);
    	    	sels[0].birthday = birthday.format("yyyy-MM-dd");
    	    	$('#content1').form('load',sels[0]);
    	    }    
    	}).window('open');
    }
},{
